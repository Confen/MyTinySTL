...
namespace B {
    struct A {
        ...
        // 方式1：友元函数声明
        friend void friend_swap1(A& a, A& b);
        
        // 方式2：友元函数内联定义（在类内部）
        friend void friend_swap2(A& a, A& b) {
        ...
        // 方式3：友元函数声明（将在全局作用域定义）
        friend void global_friend_swap(A& a, A& b);
    };
    
    // 方式1：在命名空间内定义友元函数
    void friend_swap1(A& a, A& b) {
        std::cout << "命名空间内友元函数: B::friend_swap1" << std::endl;
        int temp = a.data;
        a.data = b.data;
        b.data = temp;
    }
...
int main() {
    ...
    // 1. 内联友元函数（在类内部定义）
    std::cout << "\n1. 内联友元函数:" << std::endl;
    friend_swap2(a, b);  // ADL 找到 B::friend_swap2
    std::cout << "   交换后: a=" << a.data << ", b=" << b.data << std::endl;
    
    // 2. 命名空间内友元函数
    std::cout << "\n2. 命名空间内友元函数:" << std::endl;
    friend_swap1(a, b);  // ADL 找到 B::friend_swap1
    std::cout << "   交换后: a=" << a.data << ", b=" << b.data << std::endl;
    ...
    // 调用命名空间内的友元函数
    B::friend_swap1(a, b);
    std::cout << "   调用 B::friend_swap1 后: a=" << a.data << ", b=" << b.data << std::endl;
