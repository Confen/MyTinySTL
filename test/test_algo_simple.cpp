...
void test_non_modifying_algorithms() {
    ...
    // 测试 all_of
    bool all_even = mystl::all_of(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    std::cout << "所有元素都是偶数: " << (all_even ? "是" : "否") << std::endl;
    
    // 测试 any_of
    bool any_even = mystl::any_of(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    std::cout << "存在偶数元素: " << (any_even ? "是" : "否") << std::endl;
    
    // 测试 none_of
    bool none_negative = mystl::none_of(vec.begin(), vec.end(), [](int x) { return x < 0; });
    std::cout << "没有负数元素: " << (none_negative ? "是" : "否") << std::endl;
    
    // 测试 count
    size_t count_even = mystl::count(vec.begin(), vec.end(), 2);
    std::cout << "数字2的个数: " << count_even << std::endl;
    
    // 测试 count_if
    size_t count_even_numbers = mystl::count_if(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    ...
    // 测试 find_if
    auto it_even = mystl::find_if(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    if (it_even != vec.end()) {
        std::cout << "找到第一个偶数: " << *it_even << std::endl;
    }
    
    // 测试 find_if_not
    auto it_odd = mystl::find_if_not(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    if (it_odd != vec.end()) {
...
void test_binary_search_algorithms() {
    ...
    // 测试 lower_bound
    auto it_lower = mystl::lower_bound(vec.begin(), vec.end(), 8);
    if (it_lower != vec.end()) {
        std::cout << "lower_bound(8) 找到: " << *it_lower << std::endl;
    }
    
    // 测试 upper_bound
    auto it_upper = mystl::upper_bound(vec.begin(), vec.end(), 8);
    if (it_upper != vec.end()) {
        std::cout << "upper_bound(8) 找到: " << *it_upper << std::endl;
    }
    
    // 测试 binary_search
    bool found = mystl::binary_search(vec.begin(), vec.end(), 7);
    std::cout << "binary_search(7): " << (found ? "找到" : "未找到") << std::endl;
    
    found = mystl::binary_search(vec.begin(), vec.end(), 8);
...
void test_generating_algorithms() {
    ...
    // 测试 generate
    int counter = 1;
    mystl::generate(vec.begin(), vec.end(), [&counter]() { return counter++; });
    print_container(vec, "generate后");
    
    // 测试 generate_n
    std::vector<int> vec2(5);
    mystl::generate_n(vec2.begin(), 5, [&counter]() { return counter++; });
...
void test_sorting_algorithms() {
    ...
    // 测试 is_sorted
    bool sorted = mystl::is_sorted(vec.begin(), vec.end());
    std::cout << "是否已排序: " << (sorted ? "是" : "否") << std::endl;
    
    // 测试 max_element
    auto max_it = mystl::max_element(vec.begin(), vec.end());
    std::cout << "最大元素: " << *max_it << std::endl;
    
    // 测试 min_element
    auto min_it = mystl::min_element(vec.begin(), vec.end());
    ...
    // 测试 is_sorted
    sorted = mystl::is_sorted(vec.begin(), vec.end());
...
void test_transforming_algorithms() {
    ...
    // 测试 transform (一元操作)
    mystl::transform(vec1.begin(), vec1.end(), result.begin(), [](int x) { return x * 2; });
    print_container(result, "transform(一元)后");
    
    // 测试 transform (二元操作)
    mystl::transform(vec1.begin(), vec1.end(), vec2.begin(), result.begin(), 
...
void test_reversing_algorithms() {
    ...
    mystl::reverse_copy(vec2.begin(), vec2.end(), result.begin());
...
void test_random_algorithms() {
    ...
    // 测试 random_shuffle
    mystl::random_shuffle(vec.begin(), vec.end());
...
void test_modifying_algorithms() {
    ...
    // 测试 move
    mystl::move(vec1.begin(), vec1.end(), vec3.begin());
    print_container(vec3, "move后");
    
    // 测试 fill
    mystl::fill(vec1.begin(), vec1.end(), 0);
    print_container(vec1, "fill(0)后");
    
    // 测试 fill_n
    mystl::fill_n(vec1.begin(), 3, 1);
...
void test_partitioning_algorithms() {
    ...
    // 测试 partition
    auto it = mystl::partition(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
    print_container(vec, "partition(偶数)后");
    std::cout << "分区点位置: " << (it - vec.begin()) << std::endl;
    
    // 测试 is_partitioned
    bool partitioned = mystl::is_partitioned(vec.begin(), vec.end(), [](int x) { return x % 2 == 0; });
...
void test_heap_algorithms() {
    ...
    // 测试 push_heap
    vec.push_back(8);
    mystl::push_heap(vec.begin(), vec.end());
...
void test_merging_algorithms() {
    ...
    // 测试 merge
    mystl::merge(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), result.begin());
    print_container(result, "merge后");
    
    // 测试 inplace_merge
    std::vector<int> vec3 = {1, 3, 5, 2, 4, 6};
    print_container(vec3, "inplace_merge前");
    mystl::inplace_merge(vec3.begin(), vec3.begin() + 3, vec3.end());
...
void test_rotating_algorithms() {
    ...
    // 测试 rotate
    mystl::rotate(vec.begin(), vec.begin() + 3, vec.end());
...
int main() {
    ...
    SetConsoleCP(65001);
    std::cout << "MyTinySTL 算法库简单测试" << std::endl;
    std::cout << "=========================" << std::endl;
